...
public class TaskScheduler {
    ...
    static class Solution {
        public int leastInterval(char[] tasks, int n) {
            ...
            for(Character s : counter.keySet()) {
                int p = counter.get(s);
                pq.add(new String[]{s.toString(), String.valueOf(p)});
            }
            if(pq.size() == 1) {
                return ((Integer.valueOf(String.valueOf(pq.poll()[1])) - 1) * (n+1)) + 1;
            } else {
                int maxSize = Integer.parseInt(String.valueOf(pq.poll()[1]));
                if(maxSize * (n + 1) < tasks.length) {
                    return tasks.length;
                }
                int finalAns = (maxSize - 1) * (n + 1) + 1;
                while (pq.size() > 0) {
                    int cur = Integer.parseInt(String.valueOf(pq.poll()[1]));
                    if(cur == maxSize) {
                        finalAns += 1;
                    } else {
                        break;
